...
class CsvParser {
  ...
  static parse(csvText) {
    ...
    const colMap = {
      ...
      member: columns.findIndex(c => c === 'responsavel' || c === 'responsável' || c === 'membro' || c === 'member')
    };
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const rawMember = colMap.member !== -1 ? values[colMap.member] : 'Casal';
      ...
      // 4. Normalize Member (Paula | Alcides | Casal)
      let finalMember = 'Casal';
      if (rawMember) {
        const cleanMember = rawMember.trim();
        if (cleanMember.toLowerCase() === 'paula') {
          finalMember = 'Paula';
        } else if (cleanMember.toLowerCase() === 'alcides') {
          finalMember = 'Alcides';
        } else if (cleanMember.toLowerCase() === 'casal' || cleanMember.toLowerCase() === 'compartilhado') {
          finalMember = 'Casal';
        } else {
          errors.push(`Linha ${rowNum}: Responsável inválido "${rawMember}". Responsáveis aceitos: Paula, Alcides, Casal.`);
          continue;
        }
      }
